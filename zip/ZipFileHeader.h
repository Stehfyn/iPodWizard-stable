...
public:	
	...
	*/
	 ...
	 */
	ZIP_U32_U64 GetDataSize(bool bUseLocal = false, bool bReal = true) const
	{
		ZIP_U32_U64 uSize = bUseLocal ? m_uLocalComprSize : m_uComprSize;
		if (bReal)
	...
	*/
	...
	{
		ZIP_U32_U64 uBefore = m_uUncomprSize;
		// ignore the length of encryption info 
		ZIP_U32_U64 uAfter = GetDataSize(false, true);
	...
	*/
	...
	WORD m_uMethod;					///< The compression method. Can be 0 (storing) or \c Z_DEFLATE (deflating).
	...
	ZIP_U32_U64 m_uComprSize;		///< The compressed size.
	ZIP_U32_U64 m_uUncomprSize;		///< The uncompressed size.
	ZIP_U16_U32 m_uDiskStart;		///< The disk number at which the compressed file starts.
	WORD m_uInternalAttr;			///< Internal file attributes.
	ZIP_U32_U64 m_uLocalComprSize;	///< The compressed size written in the local header.
	ZIP_U32_U64 m_uLocalUncomprSize;///< The uncompressed size written in the local header.
	ZIP_U32_U64 m_uOffset;				///< Relative offset of the local header with respect to #m_uDiskStart.
...
protected:
	...
	*/
	...
	/**
		...
		\param	bEncrypted
			Set to \c true, if the file will be encrypted; \c false otherwise.

		\param bSegm
			Set to \c true, if the archive is segmented; \c false otherwise.
	*/
	void PrepareData(int iLevel, bool bEncrypted, bool bSegm);
	...
	*/
	...
	/**
		Reads the local file header from \a pStorage and validates the read data.

		\param	pStorage
			The storage to read the local file header from.
	...
	*/
	...
	/**
		...
		\param	pStorage
			The storage to read the central file header from.
	...
	*/
	void WriteCrc32(char* pBuf) const
	{
		...
		CBytesWriter::WriteBytes(pBuf, &uCrc, 4);
	}
