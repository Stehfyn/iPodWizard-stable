...
void CFontWnd::OnPaint()
{
	...
	if (m_pFont != NULL)
	{
		...
		{
			DWORD xoffset = (DWORD)clientRect.left;
			DWORD yoffset = (DWORD)clientRect.top;

			DWORD startOffset, endOffset, charwidth;
			SHORT width, ident;
			...
			for (int c = 0; c < m_pFont->GetNumMetrics(); c++)
			{
				...
				if (xoffset + (charwidth + 2) > (DWORD)clientRect.right)
				{
					z++;
				...
				if (yoffset + size.cy > (DWORD)clientRect.bottom)
					break;
				...
				if (z>=m_FontLine)
				{
					for (y = 0; y < size.cy; y++)
					{
						for (x = (int)startOffset; x < (int)endOffset; x++)
						{
